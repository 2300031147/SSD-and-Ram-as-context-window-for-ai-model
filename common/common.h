...
struct common_params_sampling {
    ...
    float   temp               = 0.0f;  // <= 0.0 to sample greedily, 0.0 to not output probabilities
...
struct common_params {
    ...
    std::vector<size_t> fit_params_target = std::vector<size_t>(llama_max_devices(), 64 * 1024*1024);
    ...
    // SSD secondary KV cache swap configuration
    ...
    int32_t     turboquant_v = 0;           // deprecated
