...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"-ksp", "--kv-swap-path"}, "PATH",
        "path to SSD secondary KV cache swap file (enables tiered NVMe memory offloading)",
        [](common_params & params, const std::string & value) {
            params.kv_swap_path = value;
        }
    ).set_env("LLAMA_ARG_KV_SWAP_PATH"));
    add_opt(common_arg(
        {"-ksd", "--kv-swap-drive"}, "DRIVE",
        "specific SSD drive or mount directory to allocate secondary swap storage on (e.g. /mnt/nvme, D:\\)",
        [](common_params & params, const std::string & value) {
            params.kv_swap_drive = value;
            if (params.kv_swap_path.empty()) {
                params.kv_swap_path = value;
            }
        }
    ).set_env("LLAMA_ARG_KV_SWAP_DRIVE"));
    add_opt(common_arg(
        {"-kss", "--kv-swap-size"}, "SIZE",
        "maximum SSD swap file size (e.g. 16G, 32GB, 10240M)",
        [](common_params & params, const std::string & value) {
            ...
            try {
                if (!s.empty()) {
                    params.kv_swap_size = (size_t) (std::stoull(s) * mul);
                }
            } catch (const std::exception &) {
                params.kv_swap_size = 0;
            }
        }
    ).set_env("LLAMA_ARG_KV_SWAP_SIZE"));
    add_opt(common_arg(
        {"-ksr", "--kv-swap-ram"}, "SIZE",
        "maximum RAM limit for WARM_RAM tier (e.g. 1G, 512M, 10240K) (default: 10M)",
        [](common_params & params, const std::string & value) {
            size_t mul = 1024 * 1024;
            std::string s = value;
            if (!s.empty()) {
    ...
    add_opt(common_arg(
        {"-tqk", "--turboquant-k"}, "LEVEL",
        "TurboQuant+ PolarQuant compression level for K cache (turbo4, turbo3, turbo2, none)",
        [](common_params & params, const std::string & value) {
            if (value == "turbo4" || value == "4") {
                params.turboquant_k = 4;
            } else if (value == "turbo3" || value == "3") {
                params.turboquant_k = 3;
            } else if (value == "turbo2" || value == "2") {
                params.turboquant_k = 2;
            } else {
                params.turboquant_k = -1;
            }
        }
    ).set_env("LLAMA_ARG_TURBOQUANT_K"));
    add_opt(common_arg(
        {"-tqv", "--turboquant-v"}, "LEVEL",
        "TurboQuant+ PolarQuant compression level for V cache (turbo4, turbo3, turbo2, none)",
        [](common_params & params, const std::string & value) {
            if (value == "turbo4" || value == "4") {
                params.turboquant_v = 4;
            } else if (value == "turbo3" || value == "3") {
                params.turboquant_v = 3;
            } else if (value == "turbo2" || value == "2") {
                params.turboquant_v = 2;
            } else {
                params.turboquant_v = -1;
            }
        }
    ).set_env("LLAMA_ARG_TURBOQUANT_V"));
    add_opt(common_arg(
        {"--hellaswag"},
