...
class SettingsStore {
	...
	/**
	 * Synchronize KV swap configuration to the server backend
	 */
	async syncKvSwapToServer(): Promise<void> {
		if (!browser) return;

		try {
			const enabled = Boolean(this.config[SETTINGS_KEYS.KV_SWAP_ENABLED]);
			const drive = (this.config[SETTINGS_KEYS.KV_SWAP_DRIVE] as string) || '';
			const sizeGB = Number(this.config[SETTINGS_KEYS.KV_SWAP_SIZE_GB]) || 64;
			const ramSizeGB = Number(this.config[SETTINGS_KEYS.KV_SWAP_RAM_SIZE_GB]) || 0;
			const turboK = (this.config[SETTINGS_KEYS.TURBOQUANT_K] as string) || 'turbo4';
			const turboV = (this.config[SETTINGS_KEYS.TURBOQUANT_V] as string) || 'turbo2';

			if (enabled && drive) {
				await fetch('/kv-swap/config', {
					method: 'POST',
			...
			console.error('Failed to sync KV swap settings to server:', err);
		}
	}
