...
export const SETTINGS_SECTIONS = {
	...
	KV_STORAGE: { slug: 'kv-storage', title: 'KV Cache & SSD Storage' },
...
export const SETTINGS_SECTION_SLUGS = {
	...
	KV_STORAGE: SETTINGS_SECTIONS.KV_STORAGE.slug,
...
export const SETTINGS_SECTION_TITLES = {
	...
	KV_STORAGE: SETTINGS_SECTIONS.KV_STORAGE.title,
...
export const SETTINGS_REGISTRY: SettingsSectionEntry[] = [
	...
	// KV Cache & SSD Storage
	{
		icon: Database,
		settings: [
			{
				defaultValue: false,
				help: 'When enabled, context exceeding GPU VRAM automatically overflows to your selected SSD drive. If disabled or if no drive is selected, SSD offloading stops completely.',
				key: SETTINGS_KEYS.KV_SWAP_ENABLED,
				label: 'Enable SSD Secondary Cache Offloading',
				type: SettingsFieldType.CHECKBOX
			},
			{
				defaultValue: '',
				dependsOn: SETTINGS_KEYS.KV_SWAP_ENABLED,
				help: 'Choose the exact physical SSD / NVMe drive to host your context cache. If not selected, SSD offloading will not run.',
				key: SETTINGS_KEYS.KV_SWAP_DRIVE,
				label: 'Selected SSD Storage Drive',
				options: [
					{ label: 'Select Drive...', value: '' },
					{ label: 'BG6 KIOXIA 1024GB NVMe SSD (/mnt/data - 953.9 GB)', value: '/mnt/data' },
					{ label: 'GIGABYTE AG450E500G-G NVMe SSD (/ - 463.8 GB)', value: '/' },
					{ label: 'Dedicated NVMe Secondary Mount (/mnt/nvme)', value: '/mnt/nvme' },
					{ label: 'External High-Speed USB/NVMe SSD (/media/ssd)', value: '/media/ssd' }
				],
				type: SettingsFieldType.SELECT
			},
			{
				defaultValue: 16,
				dependsOn: SETTINGS_KEYS.KV_SWAP_ENABLED,
				help: 'Maximum RAM limit for WARM_RAM tier in Gigabytes (GB). Acts as an intermediary cache between VRAM and SSD.',
				key: SETTINGS_KEYS.KV_SWAP_RAM_SIZE_GB,
				label: 'Secondary RAM Cache Capacity (GB)',
				options: [
					{ label: '1 GB', value: 1 },
					{ label: '2 GB', value: 2 },
					{ label: '4 GB', value: 4 },
			...
			},
			{
				defaultValue: 64,
				dependsOn: SETTINGS_KEYS.KV_SWAP_ENABLED,
				help: 'Maximum storage allocation on the selected SSD drive in Gigabytes (GB). Older context blocks will overflow here when VRAM is full.',
				key: SETTINGS_KEYS.KV_SWAP_SIZE_GB,
				label: 'Secondary SSD Cache Capacity (GB)',
				options: [
					{ label: '8 GB', value: 8 },
					{ label: '16 GB', value: 16 },
					{ label: '32 GB', value: 32 },
			...
			{
				defaultValue: 'turbo4',
				help: 'TurboQuant+ PolarQuant compression level for Key (K) cache. Compresses K cache without sacrificing attention accuracy (turbo4: 4-bit / 3.8x, turbo3: 3-bit / 4.6x, turbo2: 2-bit / 6.4x, none: disabled).',
				key: SETTINGS_KEYS.TURBOQUANT_K,
				label: 'TurboQuant Key (K) Compression Level',
				options: [
					{ label: 'Turbo4 (4-bit PolarQuant - 3.8x memory reduction)', value: 'turbo4' },
					{ label: 'Turbo3 (3-bit PolarQuant - 4.6x memory reduction)', value: 'turbo3' },
					{ label: 'Turbo2 (2-bit PolarQuant - 6.4x memory reduction)', value: 'turbo2' },
					{ label: 'Disabled (16-bit uncompressed)', value: 'none' }
				],
				type: SettingsFieldType.SELECT
			},
			{
				defaultValue: 'turbo2',
				help: 'TurboQuant+ PolarQuant compression level for Value (V) cache. Compresses V cache to maximize active context window (turbo4: 4-bit / 3.8x, turbo3: 3-bit / 4.6x, turbo2: 2-bit / 6.4x, none: disabled).',
				key: SETTINGS_KEYS.TURBOQUANT_V,
				label: 'TurboQuant Value (V) Compression Level',
				options: [
					{ label: 'Turbo2 (2-bit PolarQuant - 6.4x memory reduction)', value: 'turbo2' },
					{ label: 'Turbo3 (3-bit PolarQuant - 4.6x memory reduction)', value: 'turbo3' },
					{ label: 'Turbo4 (4-bit PolarQuant - 3.8x memory reduction)', value: 'turbo4' },
			...
			{
				defaultValue: 32,
				help: 'Tokens per secondary swap block (e.g. 16, 32, 64 tokens per block).',
				key: SETTINGS_KEYS.KV_SWAP_BLOCK_SIZE,
				label: 'SSD Swap Block Size (Tokens)',
				min: 16,
				max: 256,
				type: SettingsFieldType.INPUT
			}
		],
		slug: SETTINGS_SECTION_SLUGS.KV_STORAGE,
		title: SETTINGS_SECTION_TITLES.KV_STORAGE
	},
	// Display
	{
		icon: Monitor,
