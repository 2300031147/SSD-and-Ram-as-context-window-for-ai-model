<script lang="ts">
	...
	function handleSave() {
		...
		if (serverStore.isRouterMode && modelsStore.selectedModelName) {
			const extraArgs = [
				'--kv-swap-size', `${processedConfig[SETTINGS_KEYS.KV_SWAP_SIZE_GB] || '64'}G`,
				'--kv-swap-ram', `${processedConfig[SETTINGS_KEYS.KV_SWAP_RAM_SIZE_GB] || '16'}G`,
				'--ctx-size', '-1'
			];
			if (processedConfig[SETTINGS_KEYS.KV_SWAP_DRIVE]) {
				extraArgs.push('--kv-swap-drive', String(processedConfig[SETTINGS_KEYS.KV_SWAP_DRIVE]));
			}
			modelsStore.status.load(modelsStore.selectedModelName, extraArgs).catch(console.error);
		}
