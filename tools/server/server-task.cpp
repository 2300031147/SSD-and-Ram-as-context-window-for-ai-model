...
//
// server_task_result_config_kv_swap
//

json server_task_result_config_kv_swap::to_json() {
    if (!enabled) {
        return json {
            { "success", true },
            { "enabled", false },
            { "message", "SSD KV cache offloading disabled" }
        };
    }
    return json {
        { "success", true },
        ...
        { "size_gb", size_gb },
        { "turboquant_k", tq_k_str },
        { "turboquant_v", tq_v_str },
        { "turboquant_enabled", use_tq }
    };
}
