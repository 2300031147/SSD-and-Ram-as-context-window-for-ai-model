...
llama_memory_recurrent::~llama_memory_recurrent() {
    if (!ssd_meta_path.empty()) {
        try {
            llama_file file(ssd_meta_path.c_str(), "wb");
            llama_io_write_file io(&file);
            state_write(io, -1, 0);
            LLAMA_LOG_INFO("%s: saved recurrent state to %s\n", __func__, ssd_meta_path.c_str());
        } catch (const std::exception & e) {
            LLAMA_LOG_ERROR("%s: failed to save recurrent state: %s\n", __func__, e.what());
        }
    }
}

void llama_memory_recurrent::enable_ssd_swap(const std::string & swap_path) {
    ssd_swap_path = swap_path;
    if (ssd_swap_path.length() >= 4 && ssd_swap_path.substr(ssd_swap_path.length() - 4) == ".bin") {
        ssd_meta_path = ssd_swap_path;
        ssd_meta_path.replace(ssd_meta_path.length() - 4, 4, ".recurrent.meta");
    } else {
        ssd_meta_path = ssd_swap_path + ".recurrent.meta";
    }

    try {
        llama_file file(ssd_meta_path.c_str(), "rb");
        llama_io_read_file io(&file);
        state_read(io, -1, 0);
        LLAMA_LOG_INFO("%s: loaded recurrent state from %s\n", __func__, ssd_meta_path.c_str());
    } catch (const std::exception &) {
        // file probably doesn't exist, ignore
    }
}

void llama_memory_recurrent::disable_ssd_swap() {
    if (!ssd_meta_path.empty()) {
        try {
            llama_file file(ssd_meta_path.c_str(), "wb");
            llama_io_write_file io(&file);
...
void llama_memory_recurrent::seq_cp(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0, llama_pos p1) {
    ...
    if ((uint32_t) seq_id_dst < size && (uint32_t) seq_id_src < size) {
        ...
        set_rs_idx(seq_id_dst, (seq_id_src >= 0 && (size_t) seq_id_src < rs_idx.size()) ? rs_idx[seq_id_src] : 0);
    }
}
...
void llama_memory_recurrent::seq_keep(llama_seq_id seq_id) {
    ...
    for (llama_seq_id s = 0; s < (llama_seq_id) rs_idx.size(); ++s) {
        if (s != seq_id) {
            rs_idx[s] = 0;
        }
    }
