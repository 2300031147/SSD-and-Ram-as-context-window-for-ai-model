...
public:
    ...
    // check if sequence seq_id has any tokens in position range [p0, p1)
    bool seq_has_pos_range(llama_seq_id seq_id, llama_pos p0, llama_pos p1) const {
        if (seq_id < 0 || seq_id >= LLAMA_MAX_SEQ || seq_pos[seq_id].empty()) {
            return false;
        }

        auto it = seq_pos[seq_id].lower_bound(p0);
        return it != seq_pos[seq_id].end() && it->first < p1;
    }
