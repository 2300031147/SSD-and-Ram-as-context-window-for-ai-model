...
void llama_kv_cache::seq_keep(llama_seq_id seq_id) {
    ...
    const uint32_t stream_kept = seq_to_stream[seq_id];
    auto & cells = v_cells[stream_kept];
    auto & head  = v_heads[stream_kept];
    ...
    for (uint32_t i = 0; i < cells.size(); ++i) {
        ...
        if (cells.seq_has(i, seq_id)) {
            if (cells.seq_count(i) > 1) {
                // Cell is shared with other sequences (e.g. prompt sharing or zero-copy RAG).
                // Modifying its pos would corrupt the context for other sequences!
                // We cannot safely shift it, so we must detach this sequence.
...
void llama_kv_cache::seq_div(llama_seq_id seq_id, llama_pos p0, llama_pos p1, int d) {
    ...
    for (uint32_t i = 0; i < cells.size(); ++i) {
        ...
        if (cells.seq_has(i, seq_id)) {
            if (cells.seq_count(i) > 1) {
                // Cell is shared with other sequences. Modifying pos_div corrupts others.
                cells.seq_rm(i, seq_id);
                continue;
            }
            cells.pos_div(i, d);
    ...
    if (swap_mgr && !dry_run) {
        for (uint32_t s = 0; s < sinfo.n_stream(); ++s) {
            ...
            for (uint32_t i = 1; i <= n_tok; ++i) {
                const bool cell_discont = (i < n_tok && sinfo.idxs[s][i] != sinfo.idxs[s][i-1] + 1);
                const bool pos_discont = (i < n_tok && ubatch.pos && pos_idx + i < ubatch.n_tokens &&
                                          ubatch.pos[pos_idx + i] != ubatch.pos[pos_idx + i-1] + 1);
                if (i == n_tok || cell_discont || pos_discont) {
                ...
                if (hot_cell_start >= 0) {
                    if (bid.seq_id != seq_id) {
                        ...
                        for (uint32_t c = 0; c < bid.n_tokens; ++c) {
                            const uint32_t idx = hot_cell_start + c;
                            if (idx < cells.size()) {
                                if (!cells.seq_has(idx, seq_id)) {
                    ...
                    continue; // Successfully shared or already hot, skip swapping it in from SSD/RAM!
                }

                if (cells.seq_has_pos_range(seq_id, bid.pos_start, bid.pos_start + (llama_pos) bid.n_tokens)) {
                    continue; // seq_id already has tokens at these positions in VRAM
                }
                    ...
                    if (swap_mgr->swap_in_block(bid.seq_id, bid.pos_start, empty_start, k_tensors, v_tensors, sinfo.strm[s])) {
                        for (uint32_t c = 0; c < bid.n_tokens; ++c) {
                            ...
                            if (idx < cells.size()) {
                                ...
                                if (bid.seq_id != seq_id && !cells.seq_has(idx, seq_id)) {
                                    cells.seq_add(idx, seq_id);
                                }
                            }
                        }
...
ggml_tensor * llama_kv_cache::build_input_k_idxs(ggml_context * ctx, const llama_ubatch & ubatch) {
...
void set_input_kq_mask_impl(const args_set_input_kq_mask & args, T * data) {
...
template<typename T>
void set_input_kq_mask_impl(const args_set_input_kq_mask & args, T * data, bool causal) {
    if (causal) {
...
ggml_tensor * llama_kv_cache_context::build_input_k_idxs(ggml_context * ctx, const llama_ubatch & ubatch) {
...
void llama_kv_cache::disable_ssd_swap() {
    if (swap_mgr) {
        swap_mgr.reset();
    }
}
