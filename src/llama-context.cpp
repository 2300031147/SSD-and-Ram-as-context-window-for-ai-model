...
bool llama_memory_enable_ssd_swap(
        llama_memory_t mem,
    ...
    const bool disable = (!swap_path || swap_path[0] == '\0' || max_swap_size == 0);
    ...
    if (kv) {
        ...
        try {
            kv->enable_ssd_swap(swap_path, max_swap_size, max_ram_size, block_size, (llama_kv_swap_engine) engine, (llama_tq_mode) tq_mode_k, (llama_tq_mode) tq_mode_v, use_turboquant);
        } catch (const std::exception & e) {
            LLAMA_LOG_ERROR("%s: failed to enable SSD swap: %s\n", __func__, e.what());
            return false;
        }
        return true;
    }
    ...
    if (hybrid && hybrid->get_mem_attn()) {
        if (disable) {
            hybrid->get_mem_attn()->disable_ssd_swap();
            if (hybrid->get_mem_recr()) {
                hybrid->get_mem_recr()->disable_ssd_swap();
            }
            return true;
        }
        try {
            hybrid->get_mem_attn()->enable_ssd_swap(swap_path, max_swap_size, max_ram_size, block_size, (llama_kv_swap_engine) engine, (llama_tq_mode) tq_mode_k, (llama_tq_mode) tq_mode_v, use_turboquant);
        } catch (const std::exception & e) {
            LLAMA_LOG_ERROR("%s: failed to enable SSD swap: %s\n", __func__, e.what());
            return false;
        }
        ...
        if (hybrid->get_mem_recr()) {
            hybrid->get_mem_recr()->enable_ssd_swap(swap_path);
        }
    ...
    if (recr) {
        if (disable) {
            recr->disable_ssd_swap();
            return true;
        }
        recr->enable_ssd_swap(swap_path);
        return true;
    }
...
int32_t llama_get_available_drives(llama_drive_info_c * out_drives, int32_t max_drives) {
    const auto drives = llama_kv_get_available_drives();
    if (!out_drives || max_drives <= 0) {
        return (int32_t) drives.size();
    }
    const int32_t n = std::min((int32_t) drives.size(), max_drives);
    for (int32_t i = 0; i < n; ++i) {
        std::strncpy(out_drives[i].device_name, drives[i].device_name.c_str(), sizeof(out_drives[i].device_name) - 1);
        out_drives[i].device_name[sizeof(out_drives[i].device_name) - 1] = '\0';
        std::strncpy(out_drives[i].device_node, drives[i].device_node.c_str(), sizeof(out_drives[i].device_node) - 1);
        out_drives[i].device_node[sizeof(out_drives[i].device_node) - 1] = '\0';
        std::strncpy(out_drives[i].mount_path, drives[i].path.c_str(), sizeof(out_drives[i].mount_path) - 1);
        out_drives[i].mount_path[sizeof(out_drives[i].mount_path) - 1] = '\0';
        std::strncpy(out_drives[i].fs_type, drives[i].fs_type.c_str(), sizeof(out_drives[i].fs_type) - 1);
        out_drives[i].fs_type[sizeof(out_drives[i].fs_type) - 1] = '\0';
        out_drives[i].total_bytes = drives[i].total_bytes;
        out_drives[i].free_bytes  = drives[i].free_bytes;
        out_drives[i].is_ssd      = drives[i].is_ssd;
    }
    return n;
}

bool llama_validate_kv_swap_drive(const char * drive_path, size_t required_bytes, char * out_resolved_path, size_t max_len) {
    if (!drive_path) return false;
    std::string resolved;
    bool ok = llama_kv_validate_drive(std::string(drive_path), required_bytes, resolved);
    if (ok && out_resolved_path && max_len > 0) {
        std::strncpy(out_resolved_path, resolved.c_str(), max_len - 1);
