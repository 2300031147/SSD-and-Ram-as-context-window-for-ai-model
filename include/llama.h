...
extern "C" {
    ...
    // Enable SSD secondary swap backing for the memory / KV cache
    LLAMA_API bool llama_memory_enable_ssd_swap(
            llama_memory_t mem,
    ...
    struct llama_drive_info_c {
        char     device_name[256]; // Hardware model / device name (e.g. "BG6 KIOXIA 1024GB")
        char     device_node[128]; // Block device node (e.g. "/dev/nvme0n1p2")
        char     mount_path[256];  // Mount directory / drive root
        char     fs_type[64];      // Filesystem type
        uint64_t total_bytes;      // Total capacity in bytes
        uint64_t free_bytes;       // Free capacity in bytes
        bool     is_ssd;           // True if non-rotational SSD / NVMe
    };

    // Enumerate detected local storage and SSD drives on the system
    LLAMA_API int32_t llama_get_available_drives(struct llama_drive_info_c * out_drives, int32_t max_drives);

    // Validate selected SSD drive path and resolve swap container path
    LLAMA_API bool llama_validate_kv_swap_drive(const char * drive_path, size_t required_bytes, char * out_resolved_path, size_t max_len);
